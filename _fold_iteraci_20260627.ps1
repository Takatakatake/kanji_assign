# drakon型統合(2026-06-27): iteraci 迭→复(ユーザー承認・候補①)。
#   迭=iteraci専用の孤立字→完全消滅。复(反復/複製系列9メンバー: ripet反復/kopi複写/recidiv再発…)へ合流。
#   根拠: PIV語釈 iteraci/o=「Ripeto de la sama procedo」(同手順の反復)→ripet と真正同義。
...
$updates=@( ,@('iteraci','复') )   # 単項コンマ=単一要素入れ子配列フラット化防止

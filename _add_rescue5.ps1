# 第5次救済: WSL最新分解(2026-06-21)同期で露出した新規語根。bakter(←bakteri母音脱落=菌・bakter/icid)・leu^ko(白·leuko-=白血球)・cian(青·cyano=fer/o/cian)。cit=胞/poez=生 は結合形の多義sepなので _build_homonym.ps1 側で対応。tripsin=未対応(特定酵素)で据置。
...
foreach($r in $resc){ $root=$r[0];$k=$r[1]
  ...
  Write-Host ("追加: {0}→{1} band={2} F={3}" -f $root,$k,$band,$f)
}

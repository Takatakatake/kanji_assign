# drakon型統合(2026-06-28 第3弾): tac^ment 遣→队 / kliv 劈→裂 / razi 袭→攻(ユーザー承認・深いworkflow+両版grep確認で真の孤立確証)。
#   遣=tac^ment専用孤立字(nokt/o/tac^ment複合含め全て同根)→完全消滅。队(部隊系: team队/band队ᴮ/skadr队ˢᴰ/skip队ˢ/rot队ᴿ/karavan队ᴷ、19語根)へ合流。辞書「分遣隊…>>teamo」で真の同義。
#   劈=kliv専用孤立字→完全消滅。裂(裂開系: fend裂ᶠ割る/dehisk裂ᴰ裂開/krev裂ᴷ破裂/s^ir裂裂く/skism裂ˢ、31語根)へ合流。kliv=劈開=fend/dehiskの真の同義。
#   袭=razi専用孤立字→完全消滅。攻(攻撃系: atak攻/agres攻ᴬᴳ/ofensiv攻ᴼ、17語根)へ合流。razzia=略奪襲撃。既存の襲→攻(空襲aer=气/攻・奇襲surpriz=惊ˢ/攻)と整合。
#   ※mug^(muĝ 吼→鸣)はband=basic(Fundamento基本語)につき基準④で別途ユーザー裁定(保留)。
...
$updates=@( @('tac^ment','队'), @('kliv','裂'), @('razi','攻') )   # 3要素=フラット化なし

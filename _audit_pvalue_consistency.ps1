# P整合advisory監査(2026-07-16 第9回監査で新設・第10回監査で st/E/D/P 独立検査に精密化):
# _p_work.csv の k列(漢字)と st/E/D/P列(画数由来の優先度計算値)が整合しているか点検。
# k列を変更したのにP列を再計算し忘れた「旧P値残存」を可視化する。
# 列ごとに独立判定し、各行にどの列が不一致かを表示(現行61行 = st旧値45 + P=-0.2一律14 + st一致P旧値2)。
...
Import-Csv "$dir\_p_work.csv" -Encoding UTF8 | ForEach-Object {
  ...
  $E=$lat/$st                                       # E=lat文字数/総画数
  $Pexp = -([math]::Log($F+1) + 0.3*$E - 0.1*$st)  # D=st(一级はランク0)
  $Pcur=[double]$_.P
  # 列ごとに独立判定(第10回監査是正: 従来は st/P のみで E/D を独立検査していなかった)
  $mis=New-Object System.Collections.ArrayList
  if([int]$_.st -ne $st){ [void]$mis.Add(("st(csv={0}/real={1})" -f $_.st,$st)) }
  if([math]::Abs([double]$_.E-$E) -gt 0.005){ [void]$mis.Add(("E(csv={0}/real={1:F3})" -f $_.E,$E)) }
  if([int]$_.D -ne $st){ [void]$mis.Add(("D(csv={0}/real={1})" -f $_.D,$st)) }
  if([math]::Abs($Pcur-$Pexp) -gt 0.02){ [void]$mis.Add(("P(csv={0}/real={1:F3})" -f $_.P,$Pexp)) }
  if($mis.Count -gt 0){
    [void]$bad.Add(("{0}`t{1}" -f $_.root,($mis -join "`t")))
  }
}
Write-Host ("[G] P整合advisory: k列画数とst/E/D/P不一致(P≠5) = {0}行(旧P値残存=技術的負債。基本形/IDは現状安定・一括修正は诲/毒蛇の基本形が動くため個別対応)" -f $bad.Count)

# 第11次: 最終収束検証(2026-06-23 workflow w574wb0zr)で検出した哺乳類属の救済漏れ1件。
#   procion→兽  (アライグマ属 Procyon=食肉目。§4.6 哺乳類→最近一级字 兽。
#                既に viver/lutr/otari/dazipod/mirmekofag/falanger が兽へ救済済なのに procion だけ
#                救済リストにも _untargeted にも無く単純な取りこぼし。科形 procion/ed/oj⟦procion/科ᴱ⟧で
#                接尾 ed=科 だけ漢字化され幹 procion はラテン残存=非対称も解消。透明同義 lavurso(洗熊)
#                併記は viver(cibeto)/otari(orelfokoj)同様 obscure 属で救済を妨げない。
#                大文字始 Procion/o(こいぬ座α星=固有名)は §7 大文字ガードでラテン保持のまま不変。)
#   band=piv(内容は【動】【PIV】専)。既存 兽 base(他の哺乳類属)は rank で保護され不変、procion のみ識別子付き。
...
$resc=@( ,@('procion','兽','piv') )   # 単一要素の入れ子配列は単項コンマ必須(@(@(..))はフラット化される)
...
# F = lowercase root を分節に含む見出し数(case-sensitive: 大文字始固有名 Procion を除外)

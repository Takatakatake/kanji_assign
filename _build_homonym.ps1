...
$existing=@(
 ...
 @('tetra','四','tetra/,tetra/gram/o,tetra/borat/o,tetra/mer/o,tetra/tionat/o,tetra/tion/at/o,tetra/ol/o,tetra/bor/at/o','','tetra-数詞接頭辞(4)→四。鳥tetra/o=野鸡(基本形)は除外。tetra/ol→四(2026-06-22 ol精緻化)。tetra/bor/at(WSL過細分解同期)'),
 ...
 # === 2026-07-27 第15レンズ「一語根多義の裁定線」: amb行が裁定済みの第2義を、見出しが一意な派生語だけ sep で実現 ===
 #  ★構造的な穴: 下の $new は【行単位】で sep/amb を自動判定するため、bare見出し1語(plum/o 等)が
...
$new=@(
 ...
 @('kuri','居','kuri/o','【削除済ダミー・発火しない】','★2026-07-27 続39で撤回・使用禁止: 居里=Curie の部分音訳で方針R4「音訳は部分音訳も含め絶対禁止」に反する。§14続13の記録「kuritherapy(居礼=音訳で方針違反)」も同じ判断。続37でこの amb 行を語釈scopedで実現してしまったが撤回した。語釈キーを潰して不発火にしてある。★2026-07-30 文言同期: 続39直後は「kuri/o【理】は廷に戻した・要ユーザー裁定」と書いてあったが、続40のユーザー裁定で決着済み=方針書§14.2.1のとおり Curie由来(単位キュリー・元素キュリウム・キュリー療法)はラテン統一 / Curia由来(クリア・教皇庁)だけ 廷 を維持。要裁定案件ではない。将来のAIがこの行を見て 廷 を Curie義に復活させないこと'),@('fon','响','fon/o','ホン','ホン音量。fon=底と別'),
...
#   siren/male: 1見出しに第1義と第2義が同居(siren/o=【機】サイレン;【動】カイギュウ類 / male/o=木槌;【病】馬鼻疽)ゆえ第1義を維持。
#   po: po/o=【植】Poa属だが disp が 草ᴾᴼ2(草メガ群で2文字idが枯渇し数字添字)で注入不可(_homonym_disp.ps1の数字idガード)。
#       描画は _inject_final.ps1 の語釈scoped規則(Poa el|poacoj|Poace→草)で既に実現済み。 (2026-07-27 第15レンズ)
$forceAmb=@('abak','deviz','orkid','ergot','panikl','jard','prim','klik','tof','peon','plum','fon','file','topik','lab','arke','siren','male','po')
$sepN=0;$ambN=0
foreach($e in $new){ $root=$e[0];$ov=$e[1];$hw=$e[2];$kw=$e[3];$note=$e[4]
  $hw1=($hw -split ',')[0]; $hwe=[regex]::Escape($hw1)
  # ★2026-07-27 第15レンズ是正: -match は既定で【大文字小文字を無視】するため、§7固有名の見出しが
  #   「同綴2行目」として数えられ、本来 sep(=注入される)であるべき行が amb(=注入されない)へ落ちていた。
  #   実害4行: rod/o(停泊地) ↔ Rod/o(ロードス島) / siren/o ↔ Siren/o / po/o ↔ Po/o(ポー川) / male/o ↔ Male/o(マレ)。
  #   固有名は§7で必ずラテン描画になるので同綴衝突の相手にならない → -cmatch(大小区別)が正しい。
  #   これで rod/o が sep に復帰し「停泊地=啃(かじる)」の偽の友が解消する。siren/male は行自体が
  #   第1義と第2義の混在ゆえ、po は disp が数字id(草ᴾᴼ2=注入不可)ゆえ、いずれも下の $forceAmb で amb を維持する。
  $n=@($lines|Where-Object{ $_ -cmatch ("^"+$hwe+":") }).Count

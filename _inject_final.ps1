...
# 化学アルコール -ol は【分節レベル】でラテン化(下の `$s -eq 'ol'` 分岐)。語全体ラテンを廃し偽分解尊重=他分節(ment薄荷/metan沼气/retin网膜/glik糖/mono单/tri三/poli多 等)を活かす(2026-06-22)。比較ol=比は単独語(nseg=1)のみ。di/ol→二・tetra/ol→四 は homonym sep で数詞化
      ...
      for($idx=0;$idx -lt $nseg;$idx++){
        ...
        elseif(($s -eq 'klement') -and $idx -eq 0 -and ($rest -match 'mandarino')){ $tok='木'+[char]0x1D37+[char]0x1D38+[char]0x1D31; $thisMapped=$true }   # 2026-07-25 app第65R §2: 版間整合。学術版は whole-root klementin/o⟦木ᴷᴸᴱ/o⟧ だが学習者版は源が klement/in/o と分割するため klement 未割当→全latin だった。兄弟 klementin/uj/o=木ᴷᴸᴱ/器・klementin/arb/o=木ᴷᴸᴱ/树 とも不一致→分割形の先頭分節に whole-root の disp(木ᴷᴸᴱ)を鏡像適用し 木ᴷᴸᴱ/in/o に統一(新字ゼロ=既存disp再利用)。in は $segLat で latin 維持(-ine接尾。女=「クレメンの女」誤読は続3で除去済)。klement 分節は本見出し1件のみ(実データ確認)で誤爆0

...
foreach($r in @(
  ...
  #   絶対禁止」に反する(§14続13にも『kuritherapy(居礼=音訳で方針違反)』の記録があった)。
  # ★2026-07-29 文言同期: 続39 直後の「kuri/o【理】は 廷 に戻す・要ユーザー裁定」という記述が
  #   ここに残っていたが、**2026-07-27 続40 のユーザー裁定で決着済み**なので削除する。
  #   現行の確定裁定は「**Curie 由来(キュリー/キュリウム/ラジウム療法)=ラテン維持 / Curia 由来=廷**」で、
  #   実装は下方 $inlineRule の `$s -eq 'kuri'` 行(語釈gated で Curie 系を tok='kuri' に落とす)である。
  #   要ユーザー裁定の案件ではない。将来のAIがこのコメントを見て 廷 を復活させないこと。
...
#   語釈が隣にある以上ラテン化は「読める行を読めなくする」被覆の後退にしかならない(偽分解尊重の設計を壊す)。
#   撤回した群(=現状維持で正): fek/ul/o粪ᶠ/者(澱粉)・korv/et/o鸦/小(コルベット艦)・estr/ad/o长ᴱ/行(演壇)・
#   halo/o晕(もしもし)・medi/o境(メディア王国)・nomad/o牧ᴺ(命名)・sum/er/o总/粒(シュメール)・vel/ar/o帆/群(軟口蓋音)・
#   pak/tol/o包/布(パクトロス川)・parti/o党(パルティア)・part/o部(パルティア人)・roman/o小说(ローマ人)・roman/a小说・
#   sud/an/o南/员(スーダン)・rut/in/o草ᴿ/女(routine)・mar/i/o海(マリ人)・port/o/vin/o运/酒(ポートワイン)・
#   far/ad/o做/行(ファラド)・grenad/il/o手雷/具(パッションフルーツ)・lag/oftalm/o湖/目ᴼᶠ(兎眼)・silvi/kultur/o莺/耕(林業)・
#   ald/ol/o中音(アルドール)・form/aldehid/o形・fer/ik/a学ᴵ・man/o/metr/o手(圧力計)・spion/it/o受・ar/en/o群・
#   nefr/it/o肾ᴺ/炎ᵀ(ネフライト)・klor/it/o氯/盐ᴵ(緑泥石)・kord/it/o弦/炎ᵀ(コルダイト)。
#   ★是正線の更新: 「無関係な別conceptならラテン化」ではなく **「適切な既存字への置換ができる時だけ是正し、
#     できない時は現状(成分の素直な鏡像)を維持する」**。置換で残したのは下記7件のみ(いずれも被覆を落とさない):
#     pir/o/gajl/ol/o(梨→火ᴾ=台帳キー旧綴りの不発火バグ)・urin/at/o(尿/被→尿/盐ᴬ)・spor/o/trik(织ᵀᴿ→毛ᵀ)・
#     de/bat/i(从/打→从/辩ᴮ)・vulkan/iz(火山/化→硫ⱽ/化)・re/form/ator/o(具ᴬ→员ᴬᵀ)・lup/in/o(狼/女→豆ᴸᵁ/in)。
$batDebat = $hsegDisp["bat`t辩"]; if(-not $batDebat){ throw "[台帳不整合] bat→辩 の homonym 行が無い" }
$logLure = $hsegDisp["log`t诱"]; if(-not $logLure){ throw "[台帳不整合] log→诱 の homonym 行が無い" }
...
$segLat = @{ 'gram/negativ/a'=@('gram'); 'gram/pozitiv/a'=@('gram'); 'deci/bel/o'=@('bel'); 'melo/mani/o'=@('melo'); 'di/en/o'=@('di')   # di/en/o(diene=ジエン): 化学di-(=二)は神(dio=神)の偽の友→di分節のみラテン(en=-ene接尾もラテン)。di/in/o=女神(di神+in女)は神維持=正(2026-06-29 WSL同期で dien→di/en 露出)。 # lip/om/o は脂肪lip→脂の構造ルール($s -eq 'lip')へ移行(2026-06-28・lip/瘤→脂/瘤に透明化)。 人名Gram(グラム染色)=固有名→gram分節のみラテン(否/正は維持)。重量gram(克)·記録gram(图)とは別。2026-06-21。 deci/bel/o: bel=音響単位ベル(人名Bell由来)→bel分節のみラテン保持(deci=分ᴰᶜは維持=キロオームkilo/om型)。bela美(beautiful)とは別の偽の友。2026-06-27 WSL同期で decibel/o→deci/bel/o 露出。 melo/mani/o: melo=希melos(音楽)結合形→ラテン保持(mani=狂は維持)。melo/o虫ᴹ(アナグマ=甲虫属Meloe)とは同綴別語の偽の友。melodi调ᴹ(旋律)は別root。2026-06-28
  ...
  'kala-azar/o'=@('kala-azar')   # カラアザール(黒熱病 kala-azar=ヒンディー語「黒い病」)→両分節ラテン。植物 kala/o(草ᴷᴬ)・azar/o(【植】ウマノスズクサ科=草)の偽の友。2026-07-26 生物名369語根投入で azar=草 が入り 草ᴷᴬ-草ᴬᶻᴬ(草-草)へ悪化したため是正(別AI監査の指摘)。ハイフン語ゆえ見出し一意=word-scopedで安全
  ...
  # ※2026-07-26 第13レンズで一度ここへ lag/mar/silvi/port/ald/spion/ar/form/fer-ik/man の10群を
  #   追加したが、ユーザー裁定により **全て撤回**(上部の裁定コメント参照)。ラテン化は被覆の後退であり、
  #   成分の素直な鏡像を維持するのが正。置換できる既存字があった7件のみ別機構で是正済み。
...
# 化学アルコール -ol は【分節レベル】でラテン化(下の `$s -eq 'ol'` 分岐)。語全体ラテンを廃し偽分解尊重=他分節(ment薄荷/metan沼气/retin网膜/glik糖/mono单/tri三/poli多 等)を活かす(2026-06-22)。比較ol=比は単独語(nseg=1)のみ。di/ol→二・tetra/ol→四 は homonym sep で数詞化
      ...
      for($idx=0;$idx -lt $nseg;$idx++){
        ...
        # ※2026-07-26 第13レンズで鉱物/爆薬-ite の語釈scopedラテン化(nefr/klor/kord)をここに置いたが、
        #   ユーザー裁定により撤回。nefr/it/o=肾ᴺ/炎ᵀ・klor/it/o=氯/盐ᴵ・kord/it/o=弦/炎ᵀ を現状維持とする。
        #   ★実装上の教訓は残す: この種の語釈scoped規則を書く時は、必ず下の $chemSaltLine(-it→盐ᴵ)と
        #   $medIt(-itis→炎ᵀ)より【前】に置くこと。klor は $chemAcid に、nefr/kord は $medStem に入っているため、
        #   後ろに置くと上流の分岐で食われ、規則が書かれているのに1行も発火しない dead code になる(実際になった)。
        ...
        elseif(($s -eq 'kuri') -and ($rest -match 'キュリー|Transurania elemento, Cm|Terapio per radiumo')){ $tok='kuri' }   # 2026-07-27 続40 ユーザー裁定「kuri は統一してほしい。キュリー夫人から来ているので漢字割り当てしなくていい」: Curie由来(キュリー=放射能の単位・kuri/um=キュリウム96番元素・kuri/terapi=キュリー療法)は**人名由来ゆえ§7固有名の扱いでラテン統一**。R4(音訳は部分音訳も含め絶対禁止)により 居里 の 居 は使えず、意味字(射ᴷ/金)を当てると単位と元素で字が割れるため、統一してラテンにする。古代ローマのクリア/教皇庁 kuri/o(Curia=別語源)は語釈非該当で 廷 を維持
        ...
        # --- 2026-07-26 第13レンズ(ユーザー裁定後): 正本が ##偽分解(衝突語) と明示した行のうち、
        #     **適切な既存字に置換できるものだけ** 語釈scopedで是正する。ラテン退避は行わない(被覆の後退のため撤回)。
        ...
        elseif(($s -eq 'log') -and ($rest -match '魅力,魅惑,誘惑')){ $tok=$logLure; $thisMapped=$true }   # ★$rest は【コロンを含む】語釈全体($line.Substring($ci))なので ^ アンカーは効かない(初回実装で '^魅力' と書いて不発火だった)。語釈全文一致の文字列で限定する   # 2026-07-26 第14レンズ: 誘惑の名詞形 log/o(=allogo)が台帳の log→诱 語リストから漏れ、兄弟の log/i⟦诱ᴸ⟧・log/aj^/o⟦诱ᴸ/物⟧・log/il/o⟦诱ᴸ/具⟧ と食い違って 学家(学者)になっていた。log/o は同綴3行(魅惑/【海】測程器/【聖】容量単位)ゆえ word-scoped では弁別できず語釈scopedが必要。他の2行は不発火(【海】【聖】は適切な既存字が無く現状維持=2026-07-26ユーザー裁定「置換できる時だけ是正」)
        elseif(($s -eq 'metr') -and ($rest -match 'メートル（')){ $tok='米'; $thisMapped=$true }   # 2026-07-26 第14レンズ: -metr- の【単位のメートル】を 米 に固定。台帳は「非計器の-metr-は sep で米」と定めているが語リスト方式のため取りこぼしが出る(neu^ton/metr/o=N･m トルク・om/o/metr/o=Ω･m 抵抗率 が comb metr→计ᴹ に流れて『ニュートン計器』『オーム計器』になっていた。兄弟の kilo/gram/metr/o は 千ᴷᴵ/克/米 と正しく米)。両版で「語釈に『メートル（』を含み metr 分節を持つ行」は11行あり、うち9行は既に sep で米=結果不変・残る2行だけが是正される(実測)。同一見出しの om/o/metr/o【理】オーム計(計器)は語釈に『メートル（』が無く不発火=计ᴹ 維持。★この分岐は hsep/comb より【前】に置くこと(comb metr→计ᴹ に食われるため)
        ...
        elseif(($s -eq 'klement') -and $idx -eq 0 -and ($rest -match 'mandarino')){ $tok=$s }   # 2026-07-25 続16 ★撤回(ユーザー裁定A案): 続15の版間整合策 klement→木ᴷᴸᴱ を取り消し latin へ戻す。理由=ᴷᴸᴱ は語根 klementin の識別子であり、それを分節 klement に貼ると §9 の一意復号が壊れる(木ᴷᴸᴱ/in/o を逆復号すると klementin+in+o="klementinino")。klement は sidecar 未登録の偽分解断片(辞書全体で本見出し1件のみ・##偽分解(衝突語)明記・klement/a「寛大な」は本辞書に不在)。C案(klement を語根登録して正しい識別子を得る)も却下=木群k枝は44語根で飽和し klement は klementin(=kle)の完全な語幹ゆえ kle を奪って churn を起こすか 木ᴷᴸᴱᴹᴱᴺ 級の長識別子になる。B案(裸字木)も却下=断片に「木」の語義を与えてしまう。版間の粒度差(学術 whole-root klementin/o=木ᴷᴸᴱ vs 学習者 klement/in/o=latin)は §4.6「版依存の注記」が明示的に許容する現象であり、揃えるより逆復号の正しさを優先する。兄弟 klementin/uj/o=木ᴷᴸᴱ/器・klementin/arb/o=木ᴷᴸᴱ/树 は whole-root が1分節なので正しく、無変更
        ...
        elseif(($s -eq 'roman') -and ($rest -match 'ローマ人|ローマ市民|ローマ数字')){ $tok=$s }   # 2026-07-30 続55 ユーザー裁定(選択肢2): roman/o 同綴二義の語釈scoped是正。ローマ(Roma由来)→ラテン退避。小説roman(roman/o=小说)は語釈非該当で不発火=小说維持。★従来 roman/a cifer/o が ⟦小说/a 数ᶜ/o⟧=「小説の数字」、roman/a⟦小说/a⟧=「小説の」でローマ人を表し、roman/o⟦小说/o⟧【史】ローマ人 も同様の偽の友だった(3行とも正本が ##偽分解(衝突語) と明示)。★ラテンにする理由: ローマは固有名(§7)で Rome に当たる既存漢字が無く、**学術版すら rom をラテンにしている**(学術版は rom/an/a⟦rom/员/a⟧ と分解するため本規則は学術版では0行=版間が揃う)。2026-07-26裁定「置換できる時だけ是正・ラテン退避は行わない」の趣旨は第13レンズの一括掃きで被覆が大きく後退するのを避けることであり、固有名3行の限定退避はその趣旨外(ユーザー裁定で確認)。★ゲート語を'ローマ'一般でなく3語に限定した理由: 小説義の行が将来ローマに言及しても誤発火しないため(fail-safe。新しいローマ行が来たら[K]/[O]で気付く)。実測=学習者版3行にちょうど発火・誤発火0・学術版0行。Roman/i/o(ローマ帝国)は大文字ゆえ§7固有名ガードで既にラテン
        elseif(($s -eq 'at') -and ($rest -match 'Distilaj\^o de alkoholaturo')){ $tok='馏'; $thisMapped=$true }   # 2026-08-01 続62 ユーザー裁定: alkohol/at/o【薬】(アルコラトゥーラの蒸留物)は塩(盐ᴬ)でも受動分詞(被)でもない同綴第3義→既存 distil=馏 で語釈scoped分離。【化】行(Metala salo)は非該当で盐ᴬ維持。ゲートはPIV語釈の固有句=学習者版1行のみ発火(実測)
        elseif(($s -eq 'alkoholat') -and ($rest -match 'Distilaj\^o de alkoholaturo')){ $tok='醇馏'; $thisMapped=$true }   # 同上の学術版whole-root側: alkoholat/o【薬】→醇馏(化学義は醇盐維持)。学術版1行のみ発火(実測)
        elseif(($s -match '^([^!?.;,]{2,})([!?.;,]+)$') -and $ambRule.ContainsKey($Matches[1]) -and (AmbTok $Matches[1] $rest)){ $p1=$Matches[1]; $p2=$Matches[2]; $tok=((AmbTok $p1 $rest)+$p2); $thisMapped=$true; $ambN++ }   # 2026-08-01 続62 ユーザー裁定: amb参照時のみ末尾句読点を剥がす(S^ak/mat!→棋/将ᴹ!)。語義キーゲートが必須条件なので pat!(锅)等の間投詞は語釈に語義キーが無く発火しない=fail-safe(汎用のdisp側stripは意図的に行わない)
        elseif(($s -match '^([^,]{3,}),$') -and (DispHas $Matches[1])){ $tok=((DispGet $Matches[1])+','); $thisMapped=$true }   # 2026-08-01 続62: 末尾カンマ限定のdisp参照strip(tial, c^ar/tial, ke → 那因,)。末尾カンマ分節は辞書全体で tial, のみ(実測)=構造的に2見出し限定

...
$segLat = @{ 'gram/negativ/a'=@('gram'); 'gram/pozitiv/a'=@('gram'); 'deci/bel/o'=@('bel'); 'lip/om/o'=@('lip')   # 人名Gram(グラム染色)=固有名→gram分節のみラテン(否/正は維持)。重量gram(克)·記録gram(图)とは別。2026-06-21。 deci/bel/o: bel=音響単位ベル(人名Bell由来)→bel分節のみラテン保持(deci=分ᴰᶜは維持=キロオームkilo/om型)。bela美(beautiful)とは別の偽の友。2026-06-27 WSL同期で decibel/o→deci/bel/o 露出

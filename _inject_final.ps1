...
# 化学アルコール -ol は【分節レベル】でラテン化(下の `$s -eq 'ol'` 分岐)。語全体ラテンを廃し偽分解尊重=他分節(ment薄荷/metan沼气/retin网膜/glik糖/mono单/tri三/poli多 等)を活かす(2026-06-22)。比較ol=比は単独語(nseg=1)のみ。di/ol→二・tetra/ol→四 は homonym sep で数詞化
      ...
      for($idx=0;$idx -lt $nseg;$idx++){
        ...
        elseif(($s -eq 'in') -and $idx -gt 0 -and ($rest -match '【化】|alkaloid|toksin|glukozid|protein|pigment|substanc|hormon|enzim|nukleobaz')){ $tok='in' }   # 化学-ine(語釈駆動・systematic): 化学タグ【化】/アルカロイド/毒素/配糖体 を語釈に持つ -in→ラテン。女性接尾-in→女 の誤友回避。上流の化学-ine一括脱分解(strikn木ˢᴺ/in・akonit/in・efedr木ᴱᶠ/in・koni毒参/in・botul菌ᴮᵀ/in 等)に systematic 対応(語単位$chemInWordの増殖を解消)。-om→瘤と同型。母体(木/辣/菌/毒参等)は活かし -in分節のみlatin。女性-in(patr/in母・reg^/in后)は化学マーカー無で 女 維持(2026-06-27)
        elseif(($s -eq 'et') -and $idx -eq 0 -and $nseg -ge 2 -and $segs[1] -eq 'oksi'){ $tok='乙'; $thisMapped=$true }   # 化学エトキシ et/oksi(ethoxy=エチル基+酸素)→乙: 甲乙丙アルキル系列(metil甲/etil乙/propil丙)に合流。指小辞-et=小(dom/et小屋等)はidx0かつsegs[1]=oksiでないため不発火。「et+oksi(酸素)」は化学のみ=【化】タグ不要(metoksil等タグ無し参照行も捕捉)。両者漢字(latin化せず)で[F]監査非該当(2026-06-29)
        ...
        elseif(($s -eq 'himen') -and $idx -eq 0 -and ($segs -contains 'pter')){ $tok='膜'; $thisMapped=$true }   # himen/o/pter(膜翅類Hymenoptera): himen=膜(hymen=membrane)→膜(membran=膜)。標準himen/o(処女膜hymen)はpter無で处女膜(disp)維持。膜/翅=膜翅と一致(2026-06-29)
        ...
        elseif(($s -eq 'rink') -and ($segs -contains 'ornit')){ $tok='嘴'; $thisMapped=$true }   # ornit/o/rink(カモノハシOrnithorhynchus=鳥嘴): rink=嘴(rynchos=snout)。ornit(=鸟)文脈限定。鸟/嘴=鴨嘴(日本語注釈と一致)(2026-06-29)
        ...
        elseif(($s -eq 'il') -and $idx -gt 0 -and $isSysChem -and ($alkylStem.ContainsKey($segs[$idx-1]) -or (@('fen','alk','naft') -contains $segs[$idx-1]))){ $tok='基'; $thisMapped=$true }   # 化学-yl(基)→基: 化学タグ+前分節がアルキル/アリル語幹の二重ゲート。器具-il→具(paf/il銃·but/il作業具Laborilo·dek/il数列·hak/et/il包丁=化学タグ無)は不発火=具維持(2026-07-17 器具/数列誤爆是正)
        elseif(($s -eq 'an') -and $idx -gt 0 -and ($idx -lt ($nseg-1)) -and ($alkylStem.ContainsKey($segs[$idx-1]) -or (@('alk','fen','naft') -contains $segs[$idx-1]))){ $tok='an' }   # 化学-an(-ane 烷=一级外)→ラテン: 前分節がアルキル/アリル語幹時のみ。会員-an→员(krist/an)は前分節が非アルキルで不発火=员維持・対格終端-anは下(2026-07-17)

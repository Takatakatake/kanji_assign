...
# ★注意: この2ファイルは git の外にある【生きた外部データ】で、ユーザーが日記を書き足すたびに
#   伸びる。したがって [F] の実数(行数・延べ字数)は再実行のたびに動く。
#   実際 2026-08-02 の初回実行(16:00)と再実行(19:07)の間に第2集が更新され、
#   素朴集計 9,955→10,137 字・行 206→209 と変化した(割合 17.1%/30.5%/1.9% は不変)。
#   [A]〜[E] は辞書とCSVだけを見るので再現する。
